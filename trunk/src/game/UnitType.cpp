...
//    char* imagename;
    ...
    tname = new char[strlen(typeName)];
    strcpy(this->tname, typeName);
...
bool UnitType::isDoubleOwned (void) {if (doubleowned) return true; return false;}
...
std::vector<char*> UnitType::getOwners() const {
        return owners;
    }
...
Uint8 UnitType::getBuildlevel() const {
	return buildlevel;
}
