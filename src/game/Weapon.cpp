...
Weapon::Weapon(const char* wname)
{
    ...
    if (pname.size() == 0)
    {
        Logger::getInstance()->Warning(
                "Unable to find projectile for weapon '" + string(wname) + "' in inifile..");
        throw 0;
    }
    ...
    if (wheadentry == p::weappool->warheadpool.end() )
    {
        ...
        {
            Logger::getInstance()->Warning("Unable to find Warhead '" + whname + "' used for weapon '" + string(wname) + "'. Unit using this weapon will be unarmed.");
            throw 0;
        }
